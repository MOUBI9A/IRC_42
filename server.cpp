...
void Server::findCommand(int fd, std::string command, std::string param)
{
    ...
    else if (command == "PRIVMSG")
    {
        ...
        if (sendTo[0] == '#')
        {
            ...
            // std::string msg = RPL_PRIVMSG_CH(cl[0].getNickName(), cl[0].getUserName(), cl[0].getIPadd(), sendTo, message);
    ...
    {
        std::string channelName, command, ss;
        std::istringstream iss(param);
        iss >> channelName;
        ...
        {
            std::string msg2;
            std::string msg = "you have 3 options to use the bot\r\n";
            msg2 = RPL_BOT(channelName, msg);
            send_to_members(channelName, msg2, -1);
    
            msg = "1- BOT <channelName> Jock\r\n";
            msg2 = RPL_BOT(channelName, msg);
            send_to_members(channelName, msg2, -1);
            
            msg = "2- BOT <channelName> Time\r\n";
            msg2 = RPL_BOT(channelName, msg);
            send_to_members(channelName, msg2, -1);

            msg = "3- BOT <channelName> File <filePath>\r\n";
            msg2 = RPL_BOT(channelName, msg);
            send_to_members(channelName, msg2, -1);
            return;
        }
        else if(command == "Jock")
        {
            std::string msg = "Jock of the day : ";
            msg += getJoke();
            std::string msg2 = RPL_BOT(channelName, msg);
            send_to_members(channelName, msg2, -1);
            return;
        }
        else if(command == "Time")
        {
            std::string msg = "Time of the day : ";
            msg += getCurrentTime();
            std::string msg2 = RPL_BOT(channelName, msg);
            send_to_members(channelName, msg2, -1);
            return;
        ...
        {
            std::string msg = "File of the day : ";
...
void Server::executeCommand(int fd, char *buffer)
{
    ...
    while (token != NULL)
    {
        ...
        std::string command_param = command.substr(command.find(" ") + 1);// PASS THE COMMAND NAME
